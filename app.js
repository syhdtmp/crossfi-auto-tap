...
JWT_TOKENS.forEach(({ token, workerId }) => {
  ...
  function connectWebSocket() {
    socket = new WebSocket(SOCKET_URL, { headers: SOCKET_HEADERS });
    ...
    socket.onerror = (error) => {
      ...
      prettyLog(`[${workerId}] Attempting to reconnect...`);
    };
  ...
  function generateSimulationData() {
    ...
    let clicksPossible = Math.min(Math.floor(Math.random() * (50 - 30 + 1) + 30), energyBalance);
